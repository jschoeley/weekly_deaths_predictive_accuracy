    ...
    'Serfling-Poisson GLM', 'glm', list(
      formula = formula(
        observed_deaths ~
          ...
          # adjustment for last week of year
          last_iso_week*sex*age_group +
          # adjustment for first week of year
          first_iso_week*sex*age_group +
          # exposures
          offset(log(exposure))
      ),
...
dat$mod_spec %>%
  ...
  group_walk(~{
    ...
    scaler <- 1/max(fig_dat$observed_mortality)
    
    fig[[.y[[1]]]] <<-
      fig_dat %>%
      mutate(y_ob = cv_id + scaler*observed_mortality-0.5,
             y_pr = cv_id + scaler*predicted_mortality-0.5,
             y_to = scaler*observed_mortality-0.5) %>%
      ggplot(aes(x = date)) +
      geom_point(
        aes(
          group = cv_id,
      ...
      geom_line(aes(y = y_pr, group = cv_id, alpha = training),
                color = 'red') +
